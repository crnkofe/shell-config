*Vim plugini*

- [Ag.Vim](https://github.com/rking/ag.vim)
- [Ctrl-P](https://github.com/ctrlpvim/ctrlp.vim.git)
- [Jedi](https://github.com/davidhalter/jedi-vim.git)
- [NERDTree](https://github.com/scrooloose/nerdtree.git)
- PYDoc _git@github.com:fs111/pydoc.vim.git_
- Python-Mode - git://github.com/klen/python-mode.git
- [TagBar](git@github.com:majutsushi/tagbar.git)
- [Vim-Airline](https://github.com/vim-airline/vim-airline)
- [Vim-AutoFormat](git@github.com:Chiel92/vim-autoformat.git)
- [Vim-Solarized](git://github.com/altercation/vim-colors-solarized.git)
- [Vim-EasyMotion](https://github.com/Lokaltog/vim-easymotion)
- [Vim-Fugive](git://github.com/tpope/vim-fugitive.git)
- [Vim-Git](git@github.com:tpope/vim-git.git)
- [Vim-GitGutter](git://github.com/airblade/vim-gitgutter.git)
- [Vim-Go](https://github.com/fatih/vim-go.git)
- [Vim-JSBeautify](https://github.com/maksimr/vim-jsbeautify.git)
- [Vim-Lucius](git@github.com:jonathanfilip/vim-lucius.git)
- [Vim-Radon](git@github.com:rubik/vim-radon.git)
- [Vim-TaskWarrior](https://github.com/farseer90718/vim-taskwarrior)
- [Vim-Toml](git@github.com:cespare/vim-toml.git)
- [Vim-Unimpaired](git://github.com/tpope/vim-unimpaired.git)

*Aplikacije*

[Task Warrior](https://taskwarrior.org/)
[TMux](https://tmux.github.io/)

*Skripte*

- fromage | požen za editiranje
